...
var gameLoopId = null;
var gameState = 'begin';
var dark = new Image(); dark.src = 'resources/etape1/img/dark.png';
var judo = new Image(); judo.src = 'resources/etape1/img/judo.png';
var nullSquare = new Image(); nullSquare.src = 'resources/etape1/img/null.png';
 ...
 if (imageIsFit() == false) {
  if (squareMoving == null && mouse.x != null && mouse.y != null) {
   ...
   if (target != null) {
    ...
    movements++;
    if (movements >= 60 || time >= 60) {
      perdu = true;
      alert("Perdu ! Rechargez la page pour recommencer.");
    }
   }
  }
  ...
  for (i = 0, y = 0; y < 3; y++)
   for (x = 0; x < 3; x++, i++) {
    if (x == 0 && y == 0) bufferContext.drawImage(nullSquare, 0, 0);
     ...
     bufferContext.drawImage(judo, board.points[x], board.points[y], 180, 180, board.squares[i].xPos, board.squares[i].yPos, 180, 180);
